...
from data_preprocessing import preprocessed_data_US
...
def run_models(process=False, ada=False, SMOTENN=False, lstm=False, sent=False):
    ...
    # original data
    original_data_dir = ("./Datasets/US Primaries/Clean Data")
    # tokenizer fit on training set
    tokenizer_dir = ("./Datasets/US Primaries/Tokenizer")
    # pre-processed data
    proc_data_dir = ("./Datasets/US Primaries/Processed Data")
    # glove embedding
    glove_dir = ("./Datasets/LSTM paper data/glove.twitter.27B")
    data_dirs = [original_data_dir, tokenizer_dir, proc_data_dir, glove_dir]
    num_words = 50000
    if process:
        preprocessed_data_US.run_processing(num_words=num_words, length=False,
